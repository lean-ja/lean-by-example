...
ある型 `T` が期待される場所に別の型の項 `s : S` を見つけると、Lean は型エラーにする前に自動的に型変換を行うことができないか試します。ここで行われる「自動的な型変換」が型強制です。型強制を明示的に指定するには、`↑` 記号をつけて `↑x` などのようにします。なお `↑` 記号については [`[coe]`](#{root}/Reference/Attribute/Coe.md) 属性も深い関係があります。
