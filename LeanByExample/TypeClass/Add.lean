...
`Add` は `+` という二項演算子のための型クラスです。`+` 記法が何を意味するかについては制約はありません。ここまで [`HAdd`](#{root}/TypeClass/HAdd.md) と同じですが、`HAdd` は異なるかもしれない型 `α, β, γ` に対して足し算 `(· + ·) : α → β → γ` を定義することができる一方で、`Add` は同じ型 `α` に対して足し算 `(· + ·) : α → α → α` を定義することしかできません。
-/
