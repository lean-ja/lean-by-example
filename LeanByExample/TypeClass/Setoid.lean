...
`Setoid` は、与えられた型の上の同値関係を表します。具体的には、`α : Type` に対してインスタンス `sa : Setoid α` は、`α` 上の二項関係 `r : α → α → Prop` と `r` が同値関係であることの証明の組です。主として [`Quotient`](#{root}/Type/Quotient.md) の引数として使用されます。
