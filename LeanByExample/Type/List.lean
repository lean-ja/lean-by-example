...
`xs : List α` の長さは、`List.length` 関数で求めることができます。
-/

#guard [1, 2, 3].length = 3

#guard List.length ([1, 2, 3] ++ [4, 5]) = 5

/- ### 要素の取得

`List α` は [`GetElem`](#{root}/TypeClass/GetElem.md) 型クラスのインスタンスになっているため、`xs : List α` の `i : Nat` 番目の要素は `xs[i]` で取得することができます。
-/

#guard ["hello", "world"][0] = "hello"

#guard [1, 2, 3][1] = 2

/- それ以外にもサポートされている構文がありますが、詳細は [`GetElem`](#{root}/TypeClass/GetElem.md) のページを参照してください。 -/

/- ### 要素の更新

`xs : List α` の `i : Nat` 番目の要素を `x : α` に更新した新しいリストを得るには、`List.set` という関数が用意されています。
-/

#guard List.set [0, 1, 2, 3] 2 10 = [0, 1, 10, 3]
...
リスト `xs : List α` の `i` 番目の要素を削除したリストを返すには、`List.eraseIdx` 関数が用意されています。
-/

#guard List.eraseIdx [1, 2, 3] 0 = [2, 3]
#guard List.eraseIdx ["hello", "world"] 0 = ["world"]
