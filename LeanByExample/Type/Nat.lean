...
/- しかし、この方法では除算を呼び出すたびにゼロでないことを証明しなければならなくなります。現行の定義であれば、ゼロでないことをチェックするのは関数を呼び出す時ではなくて定理を適用する時だけで済みます。

返り値を [`Option`](#{root}/Type/Option.md) で包むこともできますが、そうすると割り算の結果が自然数にならないので、いちいち中身を取り出す手間が生じます。

またゼロ除算をしたときに `panic!` を呼び出してしまうという方法も考えられます。しかしこの方法では、プログラミング用途には便利でも証明には不便です。`Nat` の除算は証明にもプログラミングにも使える関数であるべきなので、例外を返すわけにいかないのです。 -/
