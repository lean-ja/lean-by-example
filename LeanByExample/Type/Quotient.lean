...
/- 元の型 `α` は、すべての項がどれかの項の同値類に属していて、複数の同値類に属する項はないので、同値類たちの直和として表されます。そこで同値類の全体のことを `α` の同値関係 `sr : Setoid α` による **商(quotient)** と呼びます。`Quotient` が表すのは、まさにこの商です。同値類を取る操作 `Quotient.mk` は、商集合への関数になります。これは恣意的なところがない、構造的に導かれる操作なので、よく **自然な(canonical)** 関数であると形容されます。
