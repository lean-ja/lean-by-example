...
実際の Lean 標準ライブラリにおける実装を紹介することはかないませんが、ここでは [`Array`](#{root}/Type/Array.md) を使った **チェイン法(chaining)** と呼ばれる方法による実装コードを紹介します。[^chaining]（むろん、例示のためのトイ実装であって実用を意図したものではありません）
