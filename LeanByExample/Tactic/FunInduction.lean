...
`fun_induction f x₁ ... xₙ` と書く代わりに、引数 `x₁ ... xₙ` を省略して `fun_induction f` と書くこともできます。引数が省略されると、Lean はゴールの中から `f` の適用箇所を探して引数を自動的に特定しようとします。ゴール中に `f` の呼び出しが 1 箇所だけあれば、引数を省略することができます。たとえば、先ほどの例では `n` を省略しても動作します。 -/
