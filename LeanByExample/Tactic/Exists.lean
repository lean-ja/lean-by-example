...
/- したがって `Exists` は単一のコンストラクタを持つ[帰納型](#{root}/Declarative/Inductive.md)なので、上記の `exists` は `exact` と[無名コンストラクタ](#{root}/Syntax/AnonymousConstructor.md)で次のように書き直すことができます。-/
