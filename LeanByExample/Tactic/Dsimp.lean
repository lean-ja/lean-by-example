...
`dsimp` は、**定義的に等しい(definitionally equal)** 変形だけを行うという制約付きの [`simp`](./Simp.md) で、一言でいえば「名前を定義に展開する」タクティクです。
...
/- ## 舞台裏
「定義的に等しい(definitionally equal)ような変形だけを行う」というのは、[`rfl`](#{root}/Tactic/Rfl.md) で示せるような命題だけを使用するという意味です。`rfl` で示せないような簡約は `dsimp` ではできません。-/
