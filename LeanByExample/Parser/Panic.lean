...
注意点として、`panic!` を使っても処理は中断しません。その部分で期待されている型の [`Inhabited`](#{root}/TypeClass/Inhabited.md) インスタンスが使用されて、値を返します。(値を返さずに終了することが許されると、パニックするプログラムを `Empty` の項と見做すことができる可能性が生じて、論理的健全性に問題が生じます。)上記の例でいうと、`Nat` の `Inhabited` インスタンスが使用されて `0` が返ります。
-/
