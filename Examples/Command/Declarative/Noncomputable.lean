...
計算可能でない関数が生じるのは、選択原理 [`Classical.choice`](./Axiom.md#ClassicalChoice) を使用したときです。選択原理は、型が「空ではない」という証明だけから、その型の項を魔法のように構成できると主張している公理です。空ではないという情報から具体的な項の情報は得られないため、選択原理を使用した関数は計算不能になります。
-/
