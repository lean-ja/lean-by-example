...
`#check` コマンドは，項の型を表示します．Lean ではすべての項に型があるので，どんな項にも使えます．`#check term` というコマンドで，`term` の型を表示します．逆に `term` の型が `T` であることを確かめるには [`example`](./Example.md) コマンドを使用して `example : T := term` とします．
