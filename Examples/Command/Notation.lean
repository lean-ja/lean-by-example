...
/- ## 優先順位
`notation` で記法を定義するときに，その記法の他の演算子などと比べたパースの優先順位(precedence)を数値で指定することができます．優先順位が高い演算子ほど，他の演算子より先に適用されます．言い換えれば，優先順位を正しく設定することにより，括弧を省略しても意図通りに式が解釈されるようにすることができます．
-/
...
-- 集合の直和の記号と被っていた．
-- 集合の直和記号は等号より優先順位が低いからエラーになっていた
#check Nat ⊕ Fin 2
